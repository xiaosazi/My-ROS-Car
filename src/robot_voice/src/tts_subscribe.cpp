...
bool do_voice(robot_voice::Voice::Request& req,
          robot_voice::Voice::Response& resp)
	{
		...
		std::cout<<"I heard :"<<req.name.c_str()<<std::endl;
		text = req.name.c_str(); 
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = b64297a5, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
	...
	ros::ServiceServer voice_server = n.advertiseService("Voice",do_voice);
    ros::spin();
exit:
	printf("按任意键退出 ...\n");
	getchar();
